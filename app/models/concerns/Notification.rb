class Notification
  ...
  def self.send_notification(session)
    if Rails.env.production? and session.android?
      ...
      self.send_android_notification(session)
    elsif Rails.env.production? and session.ios?
      self.send_ios_notification(session)
    end
    true
  end
  ...
  def self.send_ios_notification(session)
  ...
  def self.send_android_notification(session)
